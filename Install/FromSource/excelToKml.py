...
class   MainWindow( QMainWindow):
    ...
    zipColumnIndex          = 6
    countyColumnIndex       = 7
    telephoneColumnIndex    = 8
    eMailColumnIndex        = 9
    websiteColumnIndex      = 10
    latitudeColumnIndex     = 11
    longitudeColumnIndex    = 12
    excelColumnCount        = 13
#
...
#       
    ...
    #    
    def changedFirstNameData( self, newText):
        #
        self.setRowDirty( True)
        #
        return

    def changedLastNameData( self, newText):
        #
        self.setRowDirty( True)
        #
        return
    ...
    def changedCountyData( self, newText):
        #
        self.setRowDirty( True)
        #
        return
    ...
    def changedEmailData( self, newText):
        #
        self.setRowDirty( True)
        #
        return
            ...
            for s in self.readWorkBook.sheets():
                ...
                for row in range(s.nrows):
                    ...
                    for col in range(s.ncols):
                        thisCell    = s.cell(row,col)
                        thisValue   = thisCell.value
                        thisType    = thisCell.ctype
                        #
                        #   hack for zip codes
                        #
                        if thisType in (2,3): 
                            if int(thisValue) == thisValue:
                                thisValue = "%u" % int( thisValue)
                            else:
                                thisValue = "%f" % int( thisValue)
                        #
                        values.append( thisValue)
                    #
                    #   make sure each row has enough columns
                    #
                    for col in range( s.ncols, self.excelColumnCount):
                        values.append("")
                    #       
                    #       print ','.join(values)
                    #       print
...
#
    ...
    def kmlPlacemark(   self,
        ...
        #
        fullName = "%s %s" % (row[self.firstNameColumnIndex],row[self.lastNameColumnIndex])
        # 
        #   full description is the popup of the placemark
        #           
        #   it is the concat of several cols in the excel data
        #
        fullDescription = []
	#
	if 0 < len( fullName) and not fullName.isspace(): 
            fullDescription.append( fullName)        
	#
        description = row[self.descriptionColumnIndex]
        if 0 < len( description) and not description.isspace(): 
            fullDescription.append( description)
        #
        eMail = row[self.eMailColumnIndex]
        if 0 < len( eMail) and not eMail.isspace():
            mailTo = '<a href="mailto:%s"> %s </a>' % ( eMail, eMail)
            fullDescription.append( mailTo)
        #
        webSite = row[self.websiteColumnIndex]
        if 0 < len( webSite) and not webSite.isspace():
            anchor =  '<a href="%s"> %s </a>' % ( webSite, webSite)
            fullDescription.append( anchor)
        #
        street  = row[self.streetColumnIndex]
        if 0 < len( street) and not street.isspace():
            fullDescription.append( row[self.streetColumnIndex])
        #
        #   format like a postal address
        #    
        townStateZipCounty        = []    
        townStateZipCountyString  = ""
        #
        town = row[self.townColumnIndex]
        if 0 < len( town) and not town.isspace():
            townStateZipCounty.append( town)
        #
        state = row[self.stateColumnIndex]
        if 0 < len( state) and not state.isspace():
            townStateZipCounty.append( state)
        #
        zipCode = row[self.zipColumnIndex]
        if 0 < len( zipCode) and not zipCode.isspace():
            townStateZipCounty.append( zipCode)
        #
        county = row[self.countyColumnIndex]
        if 0 < len( county) and not county.isspace():
            townStateZipCounty.append( county +" Co.")
        #
        #   is there any address data
        #
        if 0 < len( townStateZipCounty):
            townStateZipString  = " ".join( townStateZipCounty)
            fullDescription.append( townStateZipString)
        #
        telephone = row[self.telephoneColumnIndex] 
        if 0 < len( telephone) and not telephone.isspace():
            fullDescription.append( telephone)
        #
...
###########################################################################################        
    ...
    def updateRowData( self):
        ...
        #
        newData = unicode( self.firstNameEdit.text())
        columnIndex = self.firstNameColumnIndex
        if( newData != thisRow[columnIndex]): 
            thisRow[columnIndex] = newData
            thisWwbSheet.write( rowIndex, columnIndex, newData)
            thisSheetDirty = True
        #    
        newData = unicode( self.lastNameEdit.text())
        columnIndex = self.lastNameColumnIndex
        if( newData != thisRow[columnIndex]): 
            thisRow[columnIndex] = newData
            thisWwbSheet.write( rowIndex, columnIndex, newData)
            thisSheetDirty = True
        ...
        if( newData != thisRow[columnIndex]): 
            ...
            thisSheetDirty = True
        #
        newData = unicode( self.countyEdit.text())
        columnIndex = self.countyColumnIndex
        if( newData != thisRow[columnIndex]): 
            thisRow[columnIndex] = newData
            thisWwbSheet.write( rowIndex, columnIndex, newData)
            thisSheetDirty = True
        ...
        if( newData != thisRow[columnIndex]): 
            ...
            thisSheetDirty = True
        #
        newData = unicode( self.eMailEdit.text())
        columnIndex = self.eMailColumnIndex
        if( newData != thisRow[columnIndex]): 
            thisRow[columnIndex] = newData
            thisWwbSheet.write( rowIndex, columnIndex, newData)
            thisSheetDirty = True
    ...
    def updateRowUi(  self):
        ...
        #
        self.firstNameEdit.setText( row[self.firstNameColumnIndex])
        self.lastNameEdit.setText(  row[self.lastNameColumnIndex])
        ...
        self.countyEdit.setText(    row[self.countyColumnIndex])
        self.telephoneEdit.setText( row[self.telephoneColumnIndex])
        self.eMailEdit.setText(     row[self.eMailColumnIndex])
    ...
    def validateColumns( self):
        ...
        for self.sheetIndex in range( len( self.sheets)):
            ...
            if ( headerRow[self.firstNameColumnIndex].lower()         
            != "first name"):
                self.colsValid  = False
                colErrorString  = "First Name"
                break
            if ( headerRow[self.lastNameColumnIndex].lower()         
            != "last name"):
                self.colsValid  = False
                colErrorString  = "Last Name"
                break
            ...
            != "zip"):
                ...
                colErrorString  = "Zip"
                break
            if ( headerRow[self.countyColumnIndex].lower()          
            != "county"):
                self.colsValid  = False
                colErrorString  = "County"
                break
            ...
            != "phone"):
                ...
                break
            if ( headerRow[self.eMailColumnIndex].lower()      
            != "e-mail"):
                self.colsValid  = False
                colErrorString  = "Email"
                break
            if ( headerRow[self.websiteColumnIndex].lower()      
            != "web site"):
                self.colsValid  = False
                colErrorString  = "Web site"
                break
    ...
    def wireUpEvents(   self): 
        ...
        #   line edits
        #               
        self.connect(   self.firstNameEdit,         SIGNAL( "textEdited( QString)"),
                        self.changedFirstNameData)
        self.connect(   self.lastNameEdit,          SIGNAL( "textEdited( QString)"),
                        self.changedLastNameData)
        ...
        self.connect(   self.zipEdit,               SIGNAL( "textEdited( QString)"),
                        self.changedZipData)
        self.connect(   self.countyEdit,            SIGNAL( "textEdited( QString)"),
                        self.changedCountyData)
        self.connect(   self.telephoneEdit,         SIGNAL( "textEdited( QString)"),
                        self.changedTelephoneData)
        self.connect(   self.eMailEdit,             SIGNAL( "textEdited( QString)"),
                        self.changedEmailData)
